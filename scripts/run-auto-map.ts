...
const fixturesRoot = path.join(repoRoot, 'tests', 'lambda', 'ai', 'fixtures', 'auto-map');
...
function usage(): string {
  return [
    'Usage:',
    '  npx tsx scripts/run-auto-map.ts [fixture-name | --all | --list | --examples | --env | --help]',
    '',
    'Options:',
...
async function runFixture(fixture: LoadedFixture): Promise<FixtureRunOutcome> {
  ...
  if (fixture.assertions.expectedStatusCode !== undefined) {
    if (response.statusCode === fixture.assertions.expectedStatusCode) {
      ...
      console.log();
      console.log(
        error(`Status assertion: FAIL (expected ${fixture.assertions.expectedStatusCode}, got ${response.statusCode})`),
      );
      failed = true;
    }
  }
...
function runEnv(): void {
  console.log(bold('Required environment variables for local auto-map runs:'));
  console.log();
  console.log('1. AI_RUNTIME_MODE=local');
  console.log('2. PROMPT_REGISTRY_LOCAL_DIR=<dir with auto-map.json>');
...
async function main(): Promise<void> {
  ...
  if (allFixtureNames.length === 0) {
    console.error(error('No fixture directories found under tests/lambda/ai/fixtures/auto-map.'));
    process.exitCode = 1;
    return;
  }

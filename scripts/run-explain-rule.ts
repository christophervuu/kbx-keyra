...
import { handler } from '../src/lambda/ai/explain-rule.js';
...
const fixturesRoot = path.join(repoRoot, 'tests', 'lambda', 'ai', 'fixtures');
...
function usage(): string {
  return [
    'Usage:',
    '  npx tsx scripts/run-explain-rule.ts [fixture-name | --all | --list | --examples | --env | --help]',
    '',
    'Options:',
...
async function loadFixture(name: string): Promise<LoadedFixture> {
  ...
  const requiredFiles = {
    ...
    request: path.join(fixtureDir, 'request.json'),
  } as const;
  ...
  const request = await readJson<unknown>(requiredFiles.request);
...
function runEnv(): void {
  console.log(bold('Required environment variables for local explain-rule runs:'));
  console.log();
  console.log('1. AI_RUNTIME_MODE=local');
  console.log('2. PROMPT_REGISTRY_LOCAL_DIR=<dir with explain-rule.json>');
...
async function main(): Promise<void> {
  ...
  if (allFixtureNames.length === 0) {
    console.error(error('No fixture directories found under tests/lambda/ai/fixtures.'));
    process.exitCode = 1;
    return;
  }

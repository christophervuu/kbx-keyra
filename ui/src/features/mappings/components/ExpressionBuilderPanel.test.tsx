...
describe('ExpressionBuilderPanel', () => {
  it('renders empty state when builderState is null', () => {
    render(<ExpressionBuilderPanel builderState={null} />);
    expect(
  ...
  it('renders empty state when selectedRule is null', () => {
    ...
    render(<ExpressionBuilderPanel builderState={state} />);
    expect(
      screen.getByText('Select a rule to edit its expression, or add a new rule.'),
    ).toBeInTheDocument();
  });
  ...
  it('renders mode toggle when rule is selected', () => {
    render(<ExpressionBuilderPanel builderState={makeBuilderState()} />);
  ...
  it('highlights the active mode button', () => {
    render(<ExpressionBuilderPanel builderState={makeBuilderState({ mode: 'editor' })} />);
  ...
  it('calls switchToBuilder when Builder button is clicked', async () => {
    ...
    render(<ExpressionBuilderPanel builderState={makeBuilderState({ switchToBuilder })} />);
  ...
  it('calls switchToEditor when Editor button is clicked (from builder mode)', async () => {
    ...
    render(
      <ExpressionBuilderPanel
        builderState={makeBuilderState({ mode: 'builder', switchToEditor })}
      />,
    );
  ...
  it('renders builder slot in builder mode', () => {
    render(<ExpressionBuilderPanel builderState={makeBuilderState({ mode: 'builder' })} />);
  ...
  it('shows unsaved changes indicator when hasUnsavedChanges is true', () => {
    render(
      <ExpressionBuilderPanel builderState={makeBuilderState({ hasUnsavedChanges: true })} />,
    );
    expect(
  ...
  it('does not show unsaved changes indicator when hasUnsavedChanges is false', () => {
    render(
      <ExpressionBuilderPanel builderState={makeBuilderState({ hasUnsavedChanges: false })} />,
    );
    expect(
  ...
  it('does not show decomposition warning when decompositionWarning is null', () => {
    render(
      <ExpressionBuilderPanel builderState={makeBuilderState({ decompositionWarning: null })} />,
    );

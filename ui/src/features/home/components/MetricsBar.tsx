...
export function MetricsBar({ metrics, loading }: MetricsBarProps) {
  ...
  const { totalProjects, totalMappings, totalSchemas, statusBreakdown } = metrics;
  const hasErrors = statusBreakdown.hasErrors > 0;
  ...
  return (
    <div role="region" aria-label="Dashboard metrics" className="flex flex-wrap gap-3">
      ...
      {/* Status breakdown card — red tint when there are errors */}
      <div
        className={`flex flex-col gap-2 rounded-lg border px-5 py-4 shadow-sm transition-colors ${
          hasErrors
            ? 'border-red-500/30 bg-red-500/10'
            : 'border-slate-700 bg-slate-900'
        }`}
        data-testid="metrics-status-card"
      >

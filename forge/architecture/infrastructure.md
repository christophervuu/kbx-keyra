...
| Auto-Map table | `AWS::DynamoDB::Table` | AutoMapTable | `integrations-keyra-auto-map` |
...
| Auto-Map orchestration state machine | `AWS::Serverless::StateMachine` | AutoMapStateMachine | `kbx-keyra-auto-map-orchestration` |
...
| AI Lambdas | Included as explicit runtime resources where feature tasks require them | Current architecture includes route-wired AI functions and Step Functions resources for supported slices (including Auto-Map async orchestration) |
...
## 16) FS-101 Auto-Map async infrastructure addendum

FS-101 adds async Auto-Map session/run infrastructure contracts.

### 16.1 Auto-Map table contract

- Dedicated `AutoMapTable` resource stores session/run/work-unit/suggestion entities.
- TTL attribute: `expiresAt`.
- Sparse GSIs are session-meta only:
  - mapping history lookup index
  - mapping open-session lookup index
- PITR is environment-parameterized (`EnableAutoMapTablePitr`) and enabled for non-disposable environments.

### 16.2 Route wiring contract

`AutoMapSessionsFunction` is route-wired for:
...
### 16.3 Execution-mode and orchestration contract

- `AUTO_MAP_EXECUTION_MODE` controls capability mode (`disabled | legacy | async`).
- `AutoMapFunction` remains the unit execution worker and may dispatch over-budget workloads to Step Functions.
- `AutoMapStateMachine` provides async orchestration path for large scopes; UI/API behavior remains invariant regardless of Lambda-vs-Step-Functions execution path.

### 16.4 IAM and least-privilege posture

...
**Recent update note (2026-05-12):** `ui-application.md` and `project-structure.md` updated for FS-051 Unified Builder Visual Shell: shared five-zone layout for ScalarFieldBuilder and ArrayBuilder; Builder/Editor toggle and overflow menu (⋮) in both builder headers; `customExpression` retired as mode card in ArrayModeSelector (now internal backing store for Editor mode); `ValidationSummaryRow` extracted as shared component; completion status added to ScalarFieldBuilder; action row normalized (Reset/Discard in both; AI buttons in scalar only). Previous: FS-048 Auto-Map Review Workspace (`EditorView` workspace mode, `useAutoMapWorkspace`, sessionStorage persistence + stale detection + refresh merge + preview integration).

...
| [backend-api.md](./backend-api.md)             | Phase 1 backend API architecture — Lambda handler conventions, API Gateway route table (19 routes), DynamoDB table schemas and access patterns, S3 content storage, standardized error envelope (including request correlation IDs and TIMEOUT code), shared utility structure, error resilience flow contract, environment variables, CORS configuration, and testing approach (unit + DynamoDB Local integration). | 2026-05-15   |

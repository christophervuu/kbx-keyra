...
| [ai-runtime.md](./ai-runtime.md)               | Shared AI runtime — prompt loading, DSL asset resolution, prompt rendering, GitHub Models invocation, structured output parsing, orchestration entry point, adapter pattern for local testing, Lambda handler conventions, and FS-065 canonical integration policy (HttpAdapter-only backend consumption, retired shortcut patterns, OpenSearch-first schema-query dependency with gated degraded fallback).   | 2026-06-02   |
...
| [backend-api.md](./backend-api.md)             | Phase 1 backend API architecture — Lambda handler conventions, API Gateway route table (revision/version endpoints included), DynamoDB/OpenSearch access patterns for schema query, S3 content storage, standardized error envelope (including request correlation IDs and TIMEOUT code), shared utility structure, error resilience flow contract, environment variables (including degraded-fallback gate), CORS configuration, and testing approach (unit + DynamoDB Local integration). | 2026-06-02   |

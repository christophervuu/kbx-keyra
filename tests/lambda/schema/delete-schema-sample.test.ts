...
async function importHandler() {
  return import('../../../src/lambda/schema/delete-schema-sample.js');
}
...
describe('delete-schema-sample handler', () => {
  beforeEach(() => {
    vi.resetModules();
    ...
    sharedMocks.parsePathParam.mockReset().mockImplementation((_event: unknown, key: string) => {
      if (key === 'id') {
        return 'schema-1';
      }
    ...
    sharedMocks.getItem.mockReset().mockResolvedValue({
      ...
      samplePayloads: [
        ...
        },
        {
          ...
          name: 'Sample 2',
          dataFormat: 'json',
          contentRef: 'schemas/schema-1/samples/sample-2/payload.json',
          usedForInference: false,
          source: 'added_sample',
          createdAt: '2026-06-09T00:00:00.000Z',
        },
    ...
    sharedMocks.updateItem.mockReset().mockResolvedValue({ schemaId: 'schema-1', samplePayloadCount: 1 });
  ...
  it('removes sample metadata and payload blob', async () => {
    ...
    expect(sharedMocks.deleteObject).toHaveBeenCalledWith({
      Bucket: 'Content',
      Key: 'schemas/schema-1/samples/sample-1/payload.json',
    });
  });

  it('returns 404 when sample does not exist', async () => {

...
var rollback_deployment_exports = {};
__export(rollback_deployment_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(rollback_deployment_exports);
...
async function handler(event) {
  ...
  try {
    ...
    const config = target.sourceType === "revision" ? await getConfig(mappingId, target.sourceNumber) : await getConfig2(mappingId, target.sourceNumber);
    if (!config) {
      return errorResponse(
...
//# sourceMappingURL=rollback-deployment.js.map

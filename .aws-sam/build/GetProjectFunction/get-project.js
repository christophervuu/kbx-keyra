...
function toMappingMetadataArray(items) {
  return items.map((item) => item).filter((item) => typeof item.mappingId === "string" && typeof item.projectId === "string" && typeof item.name === "string").map((item) => ({
    mappingId: item.mappingId,
    projectId: item.projectId,
    name: item.name,
    version: typeof item.version === "number" ? item.version : 1,
    status: item.status ?? "draft",
    sourceSchemaId: typeof item.sourceSchemaId === "string" ? item.sourceSchemaId : void 0,
    targetSchemaId: typeof item.targetSchemaId === "string" ? item.targetSchemaId : void 0,
...
async function loadSchemas(schemaIds) {
  const schemas = [];
...
async function handler(event) {
  ...
  try {
    ...
    const mappingItems = await query({
    ...
    const schemaIds = (project.schemaRefs ?? []).map((schemaRef) => schemaRef.schemaId).filter((id) => typeof id === "string" && id !== "");
    const detail = {
      ...project,
